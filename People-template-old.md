...
const shortTagId = dv.current().file.name;
const longTagId = dv.current().file.folder + '/' + dv.current().file.name;
await mentionsProcessor.run(dv, app, dv.pages('"Journal"'), shortTagId);
await mentionsProcessor.run(dv, app, dv.pages('"Journal"'), longTagId);
```
